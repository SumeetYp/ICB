<?php
    ...
    if($id!=null){
        ...
        while($row = mysqli_fetch_array($resultProspectus)){
            ...
            $output.='"'.$row["customer_id"].'":{"order_id":"'.$row['order_id'].'","product_name":"'.$row["product_name"].'", "quantity":'.$row["quantity"].', "customer_id":'.$row["customer_id"].', "customer_name":"'.$row["customer_name"].'", "whatsapp":'.$row["whatsapp"].', "email":"'.$row["email"].'", "status":"'.$status.'"},';
        }
        $output=trim($output, ',').'}';
        echo $output;
    }
